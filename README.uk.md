...
Доступні мови та шляхи до перекладів оголошені в `config.json`. Під час запуску `texts.js` читає мовні налаштування браузера, завантажує необхідні XML-файли, об’єднує вибрану мову з англійською резервною локалізацією та оновлює видимий вміст і локалізовані метадані.
...
└── res
    ...
    ├── scripts                # Локалізація, поява елементів і керування звуком
    └── texts
        ├── en/texts.xml       # Англійський вміст
        └── uk/texts.xml       # Український вміст
```
...
- Редагуйте видимі тексти в `res/texts/en/texts.xml` і `res/texts/uk/texts.xml`.
...
Інформація про походження та ліцензії сторонніх іконок зберігається поруч із відповідними ресурсами в `res/icons`.
